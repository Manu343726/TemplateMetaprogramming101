...
function(BII_INCLUDE_DATA_FILES DATA_FILES)
	if (DATA_FILES)
...
function(BII_GENERATE_EXECUTABLE USER BLOCK FNAME)
	...
	IF(BII_CREATE_LIB)
		SET(aux_src ${BII_${FNAME}_MAIN})
	ELSE()
		SET(aux_src ${BII_${FNAME}_MAIN}  ${BII_${FNAME}_SRC})
	ENDIF() 
	BII_INCLUDE_DATA_FILES("${BII_${FNAME}_MAIN_DATA_FILES}")

    ADD_EXECUTABLE( ${BII_${FNAME}_NAME} ${aux_src})
    if(BII_${USER}_${BLOCK}_INCLUDE_BIN)
        target_include_directories( ${vname} PUBLIC ${CMAKE_CURRENT_BINARY_DIR})
    endif()

    if(BII_${FNAME}_MAIN_DEPS)
        TARGET_LINK_LIBRARIES( ${vname} PUBLIC ${BII_${FNAME}_MAIN_DEPS})
        add_dependencies(${vname} ${BII_${FNAME}_MAIN_DEPS})
    endif()
    IF(BII_CREATE_LIB)
        set(LIB_NAME "${USER}_${BLOCK}")
        TARGET_LINK_LIBRARIES( ${vname} PUBLIC ${LIB_NAME})
        add_dependencies(${vname} ${LIB_NAME})
    ELSE()
        if(BII_${FNAME}_SRC_DEPS)
            TARGET_LINK_LIBRARIES( ${vname} PUBLIC ${BII_${FNAME}_SRC_DEPS})
            add_dependencies(${vname} ${BII_${FNAME}_SRC_DEPS})
        ENDIF()
    ENDIF()

    if(BII_${FNAME}_MAIN_SYSTEM_DEPS)
        HANDLE_SYSTEM_DEPS(${vname} "BII_${FNAME}_MAIN_SYSTEM_DEPS")
    endif()
    IF(NOT BII_CREATE_LIB)
        if(BII_${FNAME}_SRC_SYSTEM_DEPS)
            HANDLE_SYSTEM_DEPS(${vname} "BII_${FNAME}_SRC_SYSTEM_DEPS")
        ENDIF()
    ENDIF()
...
function(BII_GENERATE_LIB USER BLOCK)
	...
	BII_INCLUDE_DATA_FILES("${BII_LIB_DATA_FILES}")
    ...
    if(BII_LIB_SYSTEM_DEPS)
        HANDLE_SYSTEM_DEPS(${vname} "BII_LIB_SYSTEM_DEPS")
    endif()
...
function(HANDLE_SYSTEM_DEPS target_name sys_deps)
	if(${BII_IMPLICIT_RULES_ENABLED})
		foreach(sys_dep ${${sys_deps}})
			if(sys_dep STREQUAL "m")
				if(UNIX)
					target_link_libraries(${target_name} PUBLIC ${sys_dep})
				endif()
			elseif(sys_dep STREQUAL "pthread")
				if(UNIX)
					target_link_libraries(${target_name} PUBLIC ${sys_dep})
				endif()
			elseif(sys_dep STREQUAL "GL")
				if(APPLE)
			...
			elseif(sys_dep STREQUAL "GLU")
				if(UNIX)
					target_link_libraries(${target_name} PUBLIC ${sys_dep})
				elseif(WIN32)
					target_link_libraries(${target_name} PUBLIC "glu32")
				endif()
			elseif(sys_dep STREQUAL "ws2_32")
				if(WIN32)
					target_link_libraries(${target_name} PUBLIC ${sys_dep})
				endif()
			elseif(sys_dep STREQUAL "winmm")
				if(WIN32)
					target_link_libraries(${target_name} PUBLIC ${sys_dep})
				endif()
			else()
